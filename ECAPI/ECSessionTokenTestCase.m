...
- (void) testLoadAccessToken {
	...
	GHAssertFalse([session hasUnexpiredGrantToken], @"Expected session to not start from scratch and have an unexpired token");
	...
	GHAssertFalse([session hasUnexpiredGrantToken], @"Expected session to not have an unexpired token after an expired one is persisted");
	...
	[defaults synchronize];
	GHAssertTrue([session hasUnexpiredGrantToken], @"Expected session to have an unexpired token after an unexpired one is persisted");
}
